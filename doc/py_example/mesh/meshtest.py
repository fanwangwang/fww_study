...
def unique_row(a):                                                              
      tmp = np.ascontiguousarray(a).view(np.dtype((np.void, a.dtype.itemsize*a.shape[1])))
...
class TriangleMesh:
    ...
    def uniformrefine(self,n=1):
        for i in range(n):
            points = self.points
            cells = self.cells
            edge,cell2edge = self.anxstructure()
            NE = edge.shape[0]
            N = points.shape[0]
